...
<article class="grid grid-cols-3 font-mono md:container md:max-w-screen-xl p-10 bg-white rounded-2xl my-20">

	<section class="col-span-3 flex justify-evenly">
	...
	<section class="col-span-2 mr-10">
		...
		{#each experience as entry}
			<div class="experience mb-3">
				<div class="title mb-1">
					...
					<small class="text-gray-500">{entry.company} | <Icon name="map-pin" /> {entry.location}</small>
				</div>

				{#if entry.description !== ''}
					<div class="description"><SvelteMarkdown source="{entry.description}"/></div>
				{/if}
			</div>
		{/each}
	</section>
